...
class wizDB{
	...
	*/
	...
	public function __construct($dbdrv = null)
	{
		...
		try
		{
			if($this->conn == null)
			{
				...
				if($dbdrv == 'mysql')
				{
					$this->conn = new PDO($dbdrv.':host='.$DBVARS['hostname'].';dbname='.$DBVARS['db_name'],
										  $DBVARS['username'],
										  $DBVARS['password'], 
										  $opts);
				}
				elseif($dbdrv == 'sqlite')
				{
					$dbf = DOCROOT.'.secdir/dbs/'.$DBVARS['db_name'].'.sdb';
		...
		{
			$message = isset($e->xdebug_message)?$e->xdebug_message:$e->getMessage();
			logError('dberror', $message."<br/>\n");
			return null;
		}
	}
	...
	public function query($query, $params=false)
	{
		...
		//if(count($multiq)>1) foreach($multiq as $sq)$this->query($sq, $params);
	...
	public function queryAll($table, $start = 0, $count = 25, $cond = '1')
	{
		if(!$this->conn) return array();
		$qry = 'SELECT * FROM `'.$table.'` WHERE '.$cond.' LIMIT '.$start.','.$count;
	...
	{
		if(!$this->conn) return array();
		$qry = 'SELECT '.$columns.' FROM `'.$table.'` WHERE '.$this->createCond($cond);
		$stmt = $this->sendData($qry, $data);
		return $stmt->fetchAll(PDO::FETCH_ASSOC);
	}
